...
const tutorialMetadata = (folderName) => new Promise(async (resolve) => {
    let filesArray = await fs.promises.readdir(`./site/pmguides/src/${folderName}`)

    // filter out non markdown files
    let file = filesArray.filter(dirFile => dirFile.split('.').pop() === 'md');

    if(file.length > 0){
        const path = `./site/pmguides/src/${folderName}/${file[0]}`;
...
// Traverse through all the docs and convert all the markdowns to JSON
const getTutorialsMetadata = async () => {
    const tutorials = [];
    
    let tutorialsMarkdownFolderNames = fs.readdirSync('./site/pmguides/src');
