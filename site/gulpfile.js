...
// trailing slash.
const BASE_URL = args.baseUrl || "https://quickstarts.postman.com";
...
// flag.
const CODELABS_BUILD_DIR = args.codelabsDir || "pmguides/dist";

// CODELABS_SRC_DIR is the directory where the actual codelabs exist on disk.
// Despite being a constant, this can be overridden with the --codelabs-dir
// flag.
const CODELABS_SRC_DIR = args.codelabsSrcDir || "pmguides/src";
...
const CODELABS_FORMAT =
  args.codelabsFormat || "../../../claat/render/template.html";

// CODELABS_NAMESPACE is the content namespace.
const CODELABS_NAMESPACE = (args.codelabsNamespace || "guide").replace(
  /^\/|\/$/g,
  ""
);
...
const STAGING_BUCKET = gcs.bucketName(
  args.stagingBucket || "DEFAULT_STAGING_BUCKET"
);
...
gulp.task(
  "clean",
  gulp.parallel("clean:codelabs", "clean:build", "clean:dist")
);

// copy copies the built artifacts in build into dist/
gulp.task("copy:codelabs", () => {
  return gulp
    .src(CODELABS_BUILD_DIR + "/**")
...
gulp.task("build:codelabs", gulp.series("export:codelabs", "copy:codelabs"));
